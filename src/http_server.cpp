...
using namespace spiritsaway::http_utils::server;
...
{
	return do_write(create_response::not_found("route request not implemented", req_));
}
...
listener::listener(
	...
	, expire_time(in_expire_time > 1 ? in_expire_time: 1)
{
	beast::error_code ec;
...
{
	return std::make_shared<session>(std::move(socket), logger, expire_time);
}

std::shared_ptr<session> file_listener::make_session(tcp::socket&& socket)
{
	return std::make_shared<file_session>(std::move(socket), logger, expire_time, doc_root);
}